...
class ANETdetection(object):
    ...
    GROUND_TRUTH_FIELDS = ['database',  'version'] #'taxonomy',
    ...
    def _import_prediction(self, prediction_filename):
        ...
        for videoid, v in data['results'].items():
            if videoid not in self.ground_truth['video-id'].values: continue
            if videoid in self.blocked_videos:
                continue
            for result in v:
                if result['label'] not in self.activity_index.keys(): continue
    ...
    def evaluate(self,output_log):
        ...
        if self.verbose:
            ...
            print('AP in each IOU: {}'.format(self.mAP))
            print('\tAverage-mAP: {}'.format(self.mAP.mean()))
        if len(output_log) != 0:
            with open(output_log,'w') as fw:
                mAPc = self.ap.mean(axis=0)
                for activity, cidx in self.activity_index.items():
                    #cname = self.activity_index[i]
                    apc = mAPc[cidx]
                    fw.write('%s %0.4f\n'%(activity,apc))
