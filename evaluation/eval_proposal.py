...
class ANETproposal(object):
    ...
    def __init__(self, ground_truth_filename=None, proposal_filename=None,
        ...
        # Include the Block video in server (modified in 6/3/2017)
        ...
        self.blocked_videos = list()
        #print('Block Video Num: %d'%(len(self.blocked_videos)))
        
        #with open('block_video_list.txt','w') as fw:
        #    for i in range(len(self.blocked_videos)):
        #        fw.write('%s\n'%self.blocked_videos[i])
        
        self.lesstime_videos = list()
        print('The less video number: %d'%(len(self.lesstime_videos)))
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        activity_index, cidx, block_video_n = {}, 0, 0
        ...
        for videoid, v in data['database'].items():
            ...
            if videoid in self.blocked_videos:
                block_video_n += 1
                continue
            if videoid in self.lesstime_videos:
                continue
        ...
        print('time less video: %d'%(len(video_lst)))
        print('block video number: %d'%(block_video_n))
    ...
    def evaluate(self):
        ...
        if self.verbose:
            ...
            print('\tProposal 100, IOU from 0.5:0.05:0.95, Average Recall: {}%'.format(100.*float(avg_recall[-1])))
...
def average_recall_vs_avg_nr_proposals(ground_truth, proposals,
    ...
    print('max avg p: %d video_lst shape0: %d proposals_shape0: %d'%(max_avg_nr_proposals,video_lst.shape[0],proposals.shape[0]))
    print('\ttotal proposal: %d ratio: %0.4f'%(proposals.shape[0],ratio))
    ...
    print('\ttotal proposal: %d'%total_nr_proposals)
    print('\tvideo list number: %d'%video_lst.shape[0])
